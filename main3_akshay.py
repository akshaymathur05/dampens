...
'''
    #Added by Akshay
    x1_new, feature_name = analysis_traces.ngram_processing(x1,3)
    x1_key, x1_value = analysis_traces.count_top_10_patterns(x1_new, feature_name)
    analysis_traces.plot_top_10_patterns_new(x1_key, x1_value,"Malware", dir_path)
    
    x2_new, feature_name = analysis_traces.ngram_processing(x2,3)
    x2_key, x2_value = analysis_traces.count_top_10_patterns(x2_new, feature_name)
    analysis_traces.plot_top_10_patterns_new(x2_key, x2_value, "Benign", dir_path)
        
    #Find frequency of the n-gram patterns in x_key from benign and malicious traces
    #plot
    
    #End Akshay's addition
'''  
    ...
    x_train, x_test, y_train, y_test = train_test_split(x_union, y, train_size=0.7, test_size=0.3,
