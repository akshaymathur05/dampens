...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  2 13:12:32 2022
...
#Directory path to save benign and malicious pcaps
...
def create_perms_csv():
    ...
    pd.DataFrame.to_csv(pd.DataFrame(dataset), path_directory+'/Perms_Benign.csv', mode = 'a', index = False, header = False)
...
def gen_perm_vector(apk_file, pkg_name):
    ...
    try:
        ...
        vector = [] * (vec_perm+1)
        vector.append('0-'+pkg_name)
        for permission in PERMISSIONS:
            hit = 1 if permission in permissions else 0
            vector.append(hit)
        vector.insert(len(vector), 0)   #The 0 here indicates benign classification
...
def collect_pcaps(pkg_name, device):
    ...
    monkey_command = 'adb -s '+device+' shell monkey -p ' + pkg[i] + ' -c android.intent.category.LAUNCHER 1'
    ...
    tcp_command = 'tcpdump -W 1 -i wlp0s20f3 -G 90 -w '+benign_pcaps_dir_path+'/0-'+pkg_name+'.pcap' 
...
def write_to_perms_benign_csv():
    ...
    pd.DataFrame.to_csv(dataset_real[-1:], 
                        path_directory+'/Perms_Benign.csv', 
                        index = False, 
                        mode = 'a', 
                        header = False)
